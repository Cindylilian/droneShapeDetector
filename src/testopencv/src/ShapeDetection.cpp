...
void setLabel(cv::Mat& im, const std::string label, std::vector<cv::Point>& contour)
{
	...
	hoverShape.linear.x= r.x + ((r.width - text.width) / 2);
	hoverShape.linear.y= r.y + ((r.height + text.height) / 2);
	hoverShape.linear.z= 0;
	hoverShape.angular.x= 0;
	hoverShape.angular.y= 0;
	hoverShape.angular.z= 0;
	koorXShape = r.x + ((r.width - text.width) / 2);
	koorYShape = r.y + ((r.height + text.height) / 2);
	cout<<"Found a "<<label<<" Shape !\n"<<"Koordinat X Shape : "<< koorXShape <<"\nKoordinat Y Shape"<<koorYShape<<endl;
...
void process(const sensor_msgs::ImageConstPtr& cam_image){
	...
	if(otomatis && !isLanding){
		if(!gotShape){
			if(droneHeight<150 && !automateHeight){
			//naik sampai 1 meter
				command = '1';
			}else if(droneHeight>150 && !automateHeight){
				//turun sampai 1 meter
				command = '2';
			}else if(droneHeight==150&& !automateHeight){
				//stop ketika sudah 1 meter
				...
				if(waktuMajuKiri > waktu && doneGeserKiri){
	...
	// else if(gotShape && !isLandXD){
	// 		command = '5';
	// 		isLandXD = true;
	// 		isZigZag = false;
	// 		isLanding = true;
	// 	}else if(isLanding && isLandXD){
	// 		if(droneHeight > 50){
	// 			command = '2';
	// 		}else{
	// 			command = '6';
...
int main(int argc, char **argv){
	...
	hoverUp.angular.x=0.0;
	hoverUp.angular.y=0.0;
	hoverUp.angular.z=0.0;

	hoverDown.linear.x=0.0;
	hoverDown.linear.y=0.0;
	hoverDown.linear.z=-0.1;
	hoverDown.angular.x=0.0;
	hoverDown.angular.y=0.0;
	hoverDown.angular.z=0.0;

	hoverLeft.linear.x=0.0;
    hoverLeft.linear.y=0.1;
    hoverLeft.linear.z=0.0;
    hoverLeft.angular.x=0.0;
	hoverLeft.angular.y=0.0;
	hoverLeft.angular.z=0.0;

	hoverRight.linear.x=0.0;
    hoverRight.linear.y=-0.1;
    hoverRight.linear.z=0.0;
    hoverRight.angular.x=0.0;
	hoverRight.angular.y=0.0;
	hoverRight.angular.z=0.0;

	hoverForward.linear.x=0.1;
	hoverForward.linear.y=0.0;
	hoverForward.linear.z=0.0;
	hoverForward.angular.x=0.0;
	hoverForward.angular.y=0.0;
	hoverForward.angular.z=0.0;

	hoverBackward.linear.x=-0.1;
	hoverBackward.linear.y=-0.0;
	hoverBackward.linear.z=0.0;
	hoverBackward.angular.x=0.0;
	hoverBackward.angular.y=0.0;
    ...
    if(kbhit())
        ...
        case 's':
     	...
     	case 'g':
     		if (gotShape){
     			pub_empty_takeoff.publish(msg); //launches the drone
            	pub_twist.publish(hoverShape); //drone fly up
            	cout<<"Go to Image"<<endl;
     		}else{
            	cout<<"Cant found any Image"<<endl;
     		}
        	command = '~';
            break;       
		case 'l':
